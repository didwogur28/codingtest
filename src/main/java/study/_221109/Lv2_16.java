...
/*
  ...
  - 제한사항
   1) 2 ≤ cards의 길이 ≤ 100
   2) cards의 원소는 cards의 길이 이하인 임의의 자연수입니다.
   3) cards에는 중복되는 원소가 존재하지 않습니다.
   4) cards[i]는 i + 1번 상자에 담긴 카드에 적힌 숫자를 의미합니다.
...
public class Lv2_16 {
    ...
    public static void main(String[] args) {
        ...
        int solution = solution(new int[]{2,3,1});
    ...
    public static int solution(int[] cards) {
        ...
        List<Integer> scoreCnt = new ArrayList<Integer>();
        List<Integer> openBoxList = new ArrayList<>();
        ...
        for(int i=0; i<cards.length; i++) {
            ...
            while(true) {
                ...
                if(scoreList.contains(score) || openBoxList.contains(score)) {
                    break;
                }

                openBoxList.add(score);

                scoreList.add(score);
                score = cards[score-1];
            }

            scoreCnt.add(scoreList.size());

        }

        if(scoreCnt.size() > 0) {
            answer = Collections.max(scoreCnt);
            scoreCnt.remove(scoreCnt.indexOf(answer));
            answer *= Collections.max(scoreCnt);
        }
