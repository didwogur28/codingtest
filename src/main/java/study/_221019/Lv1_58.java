...
public class Lv1_58 {
    ...
    public static int[] solution(String[] id_list, String[] report, int k) {
        ...
        Set<String> reportSet = Arrays.stream(report).collect(java.util.stream.Collectors.toSet());
        ...
        for(String reSet : reportSet) {
            reportMap.get(reSet.split(" ")[1]).add(reSet.split(" ")[0]);
        }

        for(String re : reportMap.keySet()) {
            if(reportMap.get(re).size() >= k) {
                for(String id : reportMap.get(re)) {
                    reportCnt = mailCnt.get(id);
                    mailCnt.put(id, reportCnt+1);
                }
            }
        }
